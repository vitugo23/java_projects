...
public class StudentService {
     ...
     */
    /**
 * Assign a classroom to the student by setting the classroom field.
 */
public void assignClassroomToStudent(long studentId, Classroom classroom) {
    Optional<Student> optionalStudent = studentRepository.findById(studentId);
    if (optionalStudent.isPresent()) {
        Student student = optionalStudent.get();
        student.setClassroom(classroom); // ✅ Assign classroom
        studentRepository.save(student); // ✅ Persist changes
    }
}

/**
 * Return the classroom assigned to a student.
 */
public Classroom getClassroomOfStudent(long studentId) {
    Optional<Student> optionalStudent = studentRepository.findById(studentId);
    return optionalStudent.map(Student::getClassroom).orElse(null); // ✅ Get classroom or return null
}
...
public void unassignClassroomOfStudent(long studentId) {
    ...
    if (optionalStudent.isPresent()) {
        Student student = optionalStudent.get();
        student.setClassroom(null); // ✅ Unassign classroom
        studentRepository.save(student); // ✅ Persist changes
    }
}
